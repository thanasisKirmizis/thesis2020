...
from functions_suite import extract_features_series, extract_magnitude_dim, extract_smoke_series
...
#        classes = classes.loc[train_index].reset_index(drop = True)
    ...
    classes.loc[np.where(classes["Activity"] == "SmokeWK")] = 1
    classes.loc[np.where(classes["Activity"] == "SmokeGP")] = 1
    classes.loc[np.where(classes["Activity"] != 1)] = -1
    
    # Adjust the class column to the results of the sliding window
    classes = pd.DataFrame(extract_smoke_series(classes.Activity, window_len_samples, overlap, overlap_thres),
                           columns = ["Activity"])
...
for i in range(len(participants_dfs)):
    ...
    f1_list.append(f1_score(test_class, class_pred))
    recall_list.append(recall_score(test_class, class_pred))
    precision_list.append(precision_score(test_class, class_pred))
